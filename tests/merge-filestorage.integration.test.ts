...
describe("Merge Filestorage Client Integration", () => {
  ...
  it("should list files permissions", async () => {
    const { body } = await client.filestorage.files.list({
      pageSize: 1,
      withResponse: true
    });

    if (body.results && body.results.length > 0) {
      const file = body.results[0];
      expect(file.permissions).toBeDefined();
      if (file.permissions && file.permissions.length > 0) {
        const firstPermission = file.permissions[0];
  ...
  it("should retrieve file", async () => {
    const { body: filesResponse } = await client.filestorage.files.list({
      pageSize: 1,
      withResponse: true
    });

    if (filesResponse.results && filesResponse.results.length > 0) {
      const fileId = filesResponse.results[0].id;
      const { response, body } = await client.filestorage.files.retrieve(fileId, {
        withResponse: true
      });
      expect(response.status).toBe(200);
      expect(body).toBeTruthy();
  ...
  it("should retrieve drive", async () => {
    const { body: drivesResponse } = await client.filestorage.drives.list({
      pageSize: 1,
      withResponse: true
    });

    if (drivesResponse.results && drivesResponse.results.length > 0) {
      const driveId = drivesResponse.results[0].id;
      const { response, body } = await client.filestorage.drives.retrieve(driveId, {
        withResponse: true
      });
      expect(response.status).toBe(200);
      expect(body).toBeTruthy();
      expect(body.id).toBe(driveId);
    }
  });
  ...
  it("should retrieve user", async () => {
    const { body: usersResponse } = await client.filestorage.users.list({
      pageSize: 1,
      withResponse: true
    });
    ...
    if (usersResponse.results && usersResponse.results.length > 0) {
      ...
      const { response, body } = await client.filestorage.users.retrieve(userId, {
        withResponse: true
      });
      expect(response.status).toBe(200);
      expect(body).toBeTruthy();
