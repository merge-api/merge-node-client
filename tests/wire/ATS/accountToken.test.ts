...
describe("AccountTokenClient", () => {
    test("retrieve", async () => {
        ...
        server
            .mockEndpoint()
            .get("/ats/v1/account-token/public_token")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.ats.accountToken.retrieve("public_token");
    ...
    test("regenerateCreate", async () => {
        ...
        server
            .mockEndpoint()
            .post("/ats/v1/account-token/regenerate")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.ats.accountToken.regenerateCreate();
        expect(response).toEqual({
