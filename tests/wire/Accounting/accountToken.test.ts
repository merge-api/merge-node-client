...
describe("AccountTokenClient", () => {
    test("retrieve", async () => {
        ...
        server
            .mockEndpoint()
            .get("/accounting/v1/account-token/public_token")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.accounting.accountToken.retrieve("public_token");
    ...
    test("regenerateCreate", async () => {
        ...
        server
            .mockEndpoint()
            .post("/accounting/v1/account-token/regenerate")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.accounting.accountToken.regenerateCreate();
        expect(response).toEqual({
