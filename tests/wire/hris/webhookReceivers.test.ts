...
describe("WebhookReceiversClient", () => {
    test("list", async () => {
        ...
        server
            .mockEndpoint()
            .get("/hris/v1/webhook-receivers")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.hris.webhookReceivers.list();
    ...
    test("create", async () => {
        ...
        server
            .mockEndpoint()
            .post("/hris/v1/webhook-receivers")
        ...
        const response = await client.hris.webhookReceivers.create({
            event: "event",
            isActive: true,
        });
        expect(response).toEqual({
