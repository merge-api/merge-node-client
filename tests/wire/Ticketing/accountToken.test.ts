...
describe("AccountTokenClient", () => {
    test("retrieve", async () => {
        ...
        server
            .mockEndpoint()
            .get("/ticketing/v1/account-token/public_token")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.ticketing.accountToken.retrieve("public_token");
    ...
    test("regenerateCreate", async () => {
        ...
        server
            .mockEndpoint()
            .post("/ticketing/v1/account-token/regenerate")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.ticketing.accountToken.regenerateCreate();
        expect(response).toEqual({
