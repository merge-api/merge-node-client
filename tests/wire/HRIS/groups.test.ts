...
describe("GroupsClient", () => {
    ...
    test("typesList", async () => {
        ...
        const rawResponseBody = { types: ["DEPARTMENT", "COST_CENTER"] };
        server
            .mockEndpoint()
            .get("/hris/v1/groups/types")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.hris.groups.typesList({
            includeDeletedData: true,
            showEnumOrigins: "show_enum_origins",
        });
        expect(response).toEqual({
            types: ["DEPARTMENT", "COST_CENTER"],
        });
    });
});
