...
describe("AccountTokenClient", () => {
    test("retrieve", async () => {
        ...
        server
            .mockEndpoint()
            .get("/hris/v1/account-token/public_token")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.hris.accountToken.retrieve("public_token");
    ...
    test("regenerateCreate", async () => {
        ...
        server
            .mockEndpoint()
            .post("/hris/v1/account-token/regenerate")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.hris.accountToken.regenerateCreate();
        expect(response).toEqual({
