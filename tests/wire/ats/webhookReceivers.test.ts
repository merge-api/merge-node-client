...
describe("WebhookReceiversClient", () => {
    test("list", async () => {
        ...
        server
            .mockEndpoint()
            .get("/ats/v1/webhook-receivers")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.ats.webhookReceivers.list();
    ...
    test("create", async () => {
        ...
        const response = await client.ats.webhookReceivers.create({
            event: "event",
            isActive: true,
        });
        expect(response).toEqual({
