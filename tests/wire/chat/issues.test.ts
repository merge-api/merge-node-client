...
describe("IssuesClient", () => {
    test("list", async () => {
        ...
        server.mockEndpoint().get("/chat/v1/issues").respondWith().statusCode(200).jsonBody(rawResponseBody).build();

        const response = await client.chat.issues.list({
    ...
    test("retrieve", async () => {
        ...
        server.mockEndpoint().get("/chat/v1/issues/id").respondWith().statusCode(200).jsonBody(rawResponseBody).build();

        const response = await client.chat.issues.retrieve("id");
        expect(response).toEqual({
            id: "3fa85f64-5717-4562-b3fc-2c963f66afa6",
            status: "ONGOING",
            errorDescription: "Missing Permissions",
