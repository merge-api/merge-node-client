/**
 * This file was auto-generated by Fern from our API Definition.
 */

import * as serializers from "../../../index";
import * as Merge from "../../../../api/index";
import * as core from "../../../../core";

export const WebhookReceiverRequest: core.serialization.ObjectSchema<
    serializers.filestorage.WebhookReceiverRequest.Raw,
    Merge.filestorage.WebhookReceiverRequest
> = core.serialization.object({
    event: core.serialization.string(),
    isActive: core.serialization.property("is_active", core.serialization.boolean()),
    key: core.serialization.string().optional(),
});

export declare namespace WebhookReceiverRequest {
    export interface Raw {
        event: string;
        is_active: boolean;
        key?: string | null;
    }
}
