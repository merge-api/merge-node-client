...
export class IssuesClient {
     ...
     *
     * @param {Merge.chat.IssuesListRequest} request
    ...
    ): core.HttpResponsePromise<Merge.chat.PaginatedIssueList> {
    ...
    private async __list(
        request: Merge.chat.IssuesListRequest = {},
        requestOptions?: IssuesClient.RequestOptions,
    ): Promise<core.WithRawResponse<Merge.chat.PaginatedIssueList>> {
        const {
            accountToken,
            cursor,
            endDate,
        ...
        const _queryParams: Record<string, unknown> = {
            ...
            status:
                status != null
                    ? serializers.chat.IssuesListRequestStatus.jsonOrThrow(status, { unrecognizedObjectKeys: "strip" })
                    : undefined,
        };
        ...
        if (_response.ok) {
            return {
                data: serializers.chat.PaginatedIssueList.parseOrThrow(_response.body, {
        ...
        return handleNonStatusCodeError(_response.error, _response.rawResponse, "GET", "/chat/v1/issues");
    }
    ...
    public retrieve(
        id: string,
        requestOptions?: IssuesClient.RequestOptions,
    ): core.HttpResponsePromise<Merge.chat.Issue> {
    ...
    private async __retrieve(
        id: string,
        requestOptions?: IssuesClient.RequestOptions,
    ): Promise<core.WithRawResponse<Merge.chat.Issue>> {
        const _authRequest: core.AuthRequest = await this._options.authProvider.getAuthRequest();
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            _authRequest.headers,
            this._options?.headers,
        ...
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.MergeEnvironment.Production,
                `chat/v1/issues/${core.url.encodePathParam(id)}`,
            ),
            method: "GET",
            headers: _headers,
        ...
        if (_response.ok) {
            return {
                data: serializers.chat.Issue.parseOrThrow(_response.body, {
                    unrecognizedObjectKeys: "passthrough",
                    allowUnrecognizedUnionMembers: true,
                    allowUnrecognizedEnumValues: true,
                    skipValidation: true,
        ...
        return handleNonStatusCodeError(_response.error, _response.rawResponse, "GET", "/chat/v1/issues/{id}");
    }
}
