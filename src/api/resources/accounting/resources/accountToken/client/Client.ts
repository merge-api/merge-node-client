...
export class AccountToken {
     ...
     * @param {Merge.accounting.RetrieveAccountTokenRequest} request
    ...
    public regenerateCreate(
        requestOptions?: AccountToken.RequestOptions,
    ): core.HttpResponsePromise<Merge.accounting.RegenerateAccountToken> {
        return core.HttpResponsePromise.fromPromise(this.__regenerateCreate(requestOptions));
    }

    private async __regenerateCreate(
        requestOptions?: AccountToken.RequestOptions,
    ): Promise<core.WithRawResponse<Merge.accounting.RegenerateAccountToken>> {
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
        ...
        switch (_response.error.reason) {
            ...
            case "timeout":
                throw new errors.MergeTimeoutError(
                    "Timeout exceeded when calling POST /accounting/v1/account-token/regenerate.",
                );
            case "unknown":
                throw new errors.MergeError({
                    message: _response.error.errorMessage,
