...
export class AsyncPassthrough {
     ...
     *         requestFormat: Merge.filestorage.RequestFormatEnum.Json
     *     })
     */
    public async create(
        request: Merge.filestorage.DataPassthroughRequest,
        requestOptions?: AsyncPassthrough.RequestOptions
    ): Promise<Merge.filestorage.AsyncPassthroughReciept> {
        const _response = await core.fetcher({
            url: urlJoin(
                (await core.Supplier.get(this._options.environment)) ?? environments.MergeEnvironment.Production,
                "api/filestorage/v1/async-passthrough"
            ),
            method: "POST",
            headers: {
     ...
     *
     * @example
     *     await merge.filestorage.asyncPassthrough.retrieve("string")
     */
    public async retrieve(
        asyncPassthroughReceiptId: string,
        requestOptions?: AsyncPassthrough.RequestOptions
    ): Promise<Merge.filestorage.RemoteResponse> {
        const _response = await core.fetcher({
            url: urlJoin(
                (await core.Supplier.get(this._options.environment)) ?? environments.MergeEnvironment.Production,
                `api/filestorage/v1/async-passthrough/${asyncPassthroughReceiptId}`
            ),
            method: "GET",
            headers: {
                Authorization: await this._getAuthorizationHeader(),
        ...
        if (_response.ok) {
            return await serializers.filestorage.RemoteResponse.parseOrThrow(_response.body, {
                unrecognizedObjectKeys: "passthrough",
                allowUnrecognizedUnionMembers: true,
                allowUnrecognizedEnumValues: true,
                breadcrumbsPrefix: ["response"],
