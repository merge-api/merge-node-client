...
export class AsyncPassthroughClient {
    ...
    private async __create(
        ...
        return handleNonStatusCodeError(_response.error, _response.rawResponse, "POST", "/ats/v1/async-passthrough");
    }
        ...
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.MergeEnvironment.Production,
                `ats/v1/async-passthrough/${core.url.encodePathParam(async_passthrough_receipt_id)}`,
            ),
            method: "GET",
            headers: _headers,
            queryParameters: requestOptions?.queryParams,
